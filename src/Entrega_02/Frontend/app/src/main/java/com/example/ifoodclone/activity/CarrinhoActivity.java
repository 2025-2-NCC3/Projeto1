...
public class CarrinhoActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (btnFavoritos != null) btnFavoritos.setOnClickListener(v -> startActivity(new Intent(this, FavoritosActivity.class)));
        if (btnCarrinho != null) {
        ...
        apiService.checkout(body).enqueue(new Callback<CheckoutResponse>() {
            ...
            public void onResponse(Call<CheckoutResponse> call, Response<CheckoutResponse> response) {
                ...
                if (!response.isSuccessful()) {
                    try {
                        // Lê a mensagem vinda do servidor (motivo da falha)
                        String errorMsg = response.errorBody() != null
                                ? response.errorBody().string()
                                : "Falha no checkout. Tente novamente.";

                        if (response.code() == 400 && errorMsg.contains("❌")) {
                            // Mostra mensagem de estoque insuficiente
                            Toast.makeText(CarrinhoActivity.this, errorMsg, Toast.LENGTH_LONG).show();
                        } else {
                            Toast.makeText(CarrinhoActivity.this, "Falha no checkout: " + errorMsg, Toast.LENGTH_SHORT).show();
                        }
                    } catch (Exception e) {
                        Toast.makeText(CarrinhoActivity.this, "Erro no checkout. Tente novamente.", Toast.LENGTH_SHORT).show();
                    }
                    return;
                }
                ...
                if (res == null) {
                    Toast.makeText(CarrinhoActivity.this, "Resposta inválida do servidor.", Toast.LENGTH_SHORT).show();
                    return;
                }
